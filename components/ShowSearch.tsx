...
import type { TmdbTvDetails, TmdbTvSearchResult } from "@/lib/types";

type Props = {
  onPick: (show: TmdbTvDetails) => void;
  disabled?: boolean;
};

export function ShowSearch({ onPick, disabled }: Props) {
  ...
  useEffect(() => {
    ...
    setSearching(true);
    fetch(`/api/tmdb/tv-search?q=${encodeURIComponent(trimmedQuery)}`, {
  ...
  async function pick(result: TmdbTvSearchResult) {
    setPickingId(result.tmdb_id);
    try {
      const res = await fetch(`/api/tmdb/tv/${result.tmdb_id}`);
      const data = await res.json();
      if (data.show) onPick(data.show as TmdbTvDetails);
    } finally {
      setPickingId(null);
    }
  }
          ...
          {visibleResults.map((r) => (
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm text-gray-400">{r.first_air_year ?? ""}</p>
              </div>
              {pickingId === r.tmdb_id && (
                <span className="ms-auto text-xs text-gray-400">جاري الإضافة...</span>
              )}
            </button>
          ))}
        </div>
